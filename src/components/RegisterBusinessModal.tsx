...
import {
  ...
  X,
  CheckCircle2,
} from "lucide-react"; // Añadí CheckCircle2 para el botón
import {
  departamentos,
...
const RegisterBusinessModal = ({ isOpen, onClose }: Props) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-full max-w-full sm:max-w-xl p-0 rounded-2xl shadow-2xl border-0 mx-3 sm:mx-0 max-h-[90vh] overflow-y-auto">
        {" "}
        {/* Mejor responsividad y scroll en móvil */}
        {/* Header mejorado */}
        <div className="bg-gradient-to-br from-blue-50 to-indigo-700 px-5 py-4 relative flex flex-col sm:flex-row items-start sm:items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-100 text-sm mt-0.5 opacity-90 max-w-xs">
                {" "}
                {/* Texto más legible */}
        ...
        <form
          ...
          className="px-4 py-4 sm:px-6 sm:py-5 space-y-6 bg-gray-50"
        >
          {" "}
          {/* Padding responsive */}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200 mt-6">
            {" "}
            {/* Separador sutil y stack en móvil */}
            <Button
              type="button"
              variant="outline"
              onClick={onClose}
